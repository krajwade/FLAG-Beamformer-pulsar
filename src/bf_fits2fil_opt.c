...
void Float2Byte(float *pfbuf, int ilen,unsigned char *pcbuf, int sample_num){
 ...
 for (i = sample_num*ilen; i < (sample_num + 1)*ilen; ++i)
 {
     if (abs(pfbuf[i] - pfbuf[200]) > 500.0)
     {
         pfbuf[i] = pfbuf[i - 1];	
     }
 }
...
//#pragma omp for
 for (j = sample_num*ilen; j < (sample_num + 1) *ilen; ++j){
...
/* Check the size */ 
 ...
 beam0_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam1_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam2_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam3_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam4_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam5_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
 beam6_8bit =(unsigned char*)malloc(sizeof(unsigned char)*TOT_CHANS*nrows*NUM_SAMPS_PER_BLOCK);
...
/* Converting the spliced files to filterbank file for each beam */
 ...
 for(k=1; k<nrows-1;k++)
 {
    ...
    for (i=0; i<NUM_SAMPS_PER_BLOCK;i++)
    {
      ...
      if (flip_flag == 1)
	    {
		channel_flip_float(beam0, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam1, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam2, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam3, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam4, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam5, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		channel_flip_float(beam6, TOT_CHANS*NUM_SAMPS_PER_BLOCK,sample , TOT_CHANS);
		++sample;
	    }
      ...
      if (quant_flag == 1)
      {
	   Float2Byte(beam0,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam0_8bit, blocknum);
	   Float2Byte(beam1,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam1_8bit, blocknum);
	   Float2Byte(beam2,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam2_8bit, blocknum);
	   Float2Byte(beam3,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam3_8bit, blocknum);
	   Float2Byte(beam4,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam4_8bit, blocknum);
	   Float2Byte(beam5,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam5_8bit, blocknum);
	   Float2Byte(beam6,TOT_CHANS*NUM_SAMPS_PER_BLOCK, beam6_8bit, blocknum);
	    ++blocknum;
      }
      ...
      if (filt_flag == 1 && quant_flag == 1)
      {
          run_madfilter(beam0_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam1_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam2_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam3_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam4_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam5_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
          run_madfilter(beam6_8bit, TOT_CHANS*NUM_SAMPS_PER_BLOCK, TOT_CHANS,blocknum);
      }
...
{
  fwrite(beam0_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f1);
  fwrite(beam1_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f2);
  fwrite(beam2_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f3);
  fwrite(beam3_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f4);
  fwrite(beam4_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f5);
  fwrite(beam5_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f6);
  fwrite(beam6_8bit,NUM_SAMPS_PER_BLOCK*nrows*TOT_CHANS,sizeof(unsigned char),f7);
